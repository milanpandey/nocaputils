...
export default function CompressPdfClient() {
  ...
  // Compression Slider & Preset State
  const [compressionLevel, setCompressionLevel] = useState<number>(50); // 0 (Low) to 100 (High)
  ...
  const handleFileSelect = async (selectedFile: File) => {
    ...
    setStatusMsg("Reading PDF structure & page count...");
    ...
    try {
      ...
      setError("Failed to read PDF file. Please ensure it is not password protected.");
    } finally {
      setIsProcessing(false);
    }
  };

  // Compression Estimator Math
  const estimateReductionPct = Math.round(20 + (compressionLevel / 100) * 55); // 20% to 75%
  const estimatedSizeBytes = file ? Math.round(file.size * (1 - estimateReductionPct / 100)) : 0;

  // Compress PDF execution
  const compressPdf = useCallback(async () => {
    if (!file) return;

    setIsProcessing(true);
    setError(null);
    setStatusMsg("Loading PDF compressor engine...");
    ...
    try {
      ...
      // Derive scale & quality from compressionLevel slider (0 = Low Comp, 100 = High Comp)
      const scale = 1.6 - (compressionLevel / 100) * 0.7; // 1.6 down to 0.9
      const jpgQuality = 0.85 - (compressionLevel / 100) * 0.35; // 0.85 down to 0.50
      ...
      setError("Failed to compress PDF file.");
    } finally {
      setIsProcessing(false);
    }
        ...
        <main className="flex flex-1 flex-col items-center">
          <div className="mb-10 text-center max-w-3xl">
            ...
            <p className="text-lg font-bold text-[var(--text-soft)]">
              Shrink PDF file size in your browser memory with interactive quality controls and real-time size reduction estimates.
            </p>
          </div>
          ...
          {/* Interactive Compression Controls & Estimator */}
          {file && (
          ...
          <div className="w-full max-w-4xl neo-panel bg-[var(--bg-panel-muted)] p-6 text-center text-xs font-bold uppercase tracking-wider text-[var(--text-soft)] mb-12">
            🔒 <strong>100% Private Processing:</strong> PDF size reduction executes locally inside your browser memory. Zero server uploads.
          </div>
        </main>
